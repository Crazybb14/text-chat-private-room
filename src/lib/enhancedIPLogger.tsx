class EnhancedIPLogger {
  ...
  // Check if user is likely the same person (even if cookies cleared)
  isSameUser(otherDeviceId: string, otherFingerprint: string): boolean {
    // First check exact device ID match
    if (this.deviceId === otherDeviceId) {
      return true;
    }

    // Then check fingerprint similarity
    const similarity = this.calculateFingerprintSimilarity(this.fingerprint, otherFingerprint);
