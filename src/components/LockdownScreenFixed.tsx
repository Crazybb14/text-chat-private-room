...
const LockdownScreenFixed = ({ isAdmin }: { isAdmin: boolean }) => {
  ...
  useEffect(() => {
    const loadSettings = async () => {
      try {
        ...
        setSettings({
          lockdown_active: map.lockdown_active || "false",
          lockdown_message: map.lockdown_message || "System is currently locked down. Please try again later.",
          lockdown_image_url: map.lockdown_image_url || "",
          lockdown_allow_admin: map.lockdown_allow_admin || "true"
        });
      } catch (error) {
...
export default LockdownScreenFixed;